...
def download_data(tickers, start_date, end_date):
    """
    Downloads historical close prices for a list of tickers safely.
    Handles recent yfinance API changes robustly.
    """
    print(f"Downloading data for {len(tickers)} tickers...")
    raw_data = yf.download(tickers, start=start_date, end=end_date)
    
    # yfinance frequently changes column structures. We use a try/except 
    # block to gracefully handle whichever format the API returns today.
    try:
...
def backtest_pair(data, stock1, stock2, window=21, z_entry=2.0, z_exit=0.0):
    ...
    # FIX: Use .iloc[0] to satisfy Pandas 2.0+ strict positional indexing
...
if __name__ == "__main__":
    # Added V and MA (Visa and Mastercard) which are famously cointegrated
    tickers = ['JPM', 'BAC', 'C', 'WFC', 'GS', 'MS', 'V', 'MA']
    
    # Shortened the timeframe slightly to capture post-COVID market regimes
    ...
    # We slightly relaxed the significance level to 0.10 to ensure more pairs are caught
    cointegrated_pairs = find_cointegrated_pairs(historical_data, significance_level=0.10)
